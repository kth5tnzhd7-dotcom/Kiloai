...
| Today | Added SQLite database (Drizzle ORM) for persistence. Deploy tab: cPanel integration (upload ZIP via API token), manual URL entry, temp URL generation, "Copy for Cloak" button. Dashboard: 5 tabs (Links, Domains, Uploads, Site Cloner, Deploy). |
| Today | Backup/Restore: export all data as JSON, import to restore. Multi-Domain Redirect Chain: 4-step redirect with bot blocking at each step, Nicegram ad integration, step-by-step verification pages. Dashboard: 7 tabs (Links, Domains, Uploads, Cloner, Deploy, Chain, Backup). |
